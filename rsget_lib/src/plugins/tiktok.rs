...
impl Streamable for TikTok {
    fn new(url: String) -> Result<Box<TikTok>, StreamError> {
        ...
        let url_re = Regex::new(
            r"^(?:https?://)?(?:www\.)?(?:m\.)?tiktok\.com/v/([a-zA-Z0-9]+)(?:\.html)?",
        )?;
        let json_re = Regex::new(r"var data = (\{.+\});")?;
        let id_cap = url_re.captures(&url).ok_or_else(|| {
            StreamError::Rsget(RsgetError::new("[TIKTOK] Regex did not find any video id"))
        })?;
        let json_cap = json_re.captures(&res).ok_or_else(|| {
            StreamError::Rsget(RsgetError::new("[TIKTOK] Regex did not find any json"))
        })?;
        debug!("{}", &json_cap[1]);
    ...
    fn get_stream(&self) -> Result<StreamType, StreamError> {
        Ok(StreamType::Chuncked(
            self.client
                .rclient
                .get(&self.tiktok.video.download_addr.url_list[0])
                .build()?,
        ))
    }
