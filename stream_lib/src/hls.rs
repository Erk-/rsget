...
impl HlsWatch {
    fn new(request: Request, http: ReqwestClient) -> (Self, UnboundedReceiver<HlsQueue>) {
        ...
        let master_url = request.url().clone().join(".").expect("Could not join url with '.'.");
        (HlsWatch {
            tx,
            request,
            http,
...
// TODO: Rewrite when https://github.com/tokio-rs/tokio/pull/2052 is merged.
#[inline]
async fn download_to_file<AW>(
    ...
    where
        AW: AsyncWrite + Unpin,
{
    let mut source = client.execute(request).await?.bytes_stream();
    let mut size = 0;
    while let Some(item) = source.next().await {
        let item = item?;
        size += writer.write(&item[..]).await? as u64    ;
    }
